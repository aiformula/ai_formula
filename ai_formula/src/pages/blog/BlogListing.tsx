...
const ICON_MAP: Record<string, JSX.Element> = {
  ...
  "?ôÔ?": <Settings className="h-8 w-8 text-gray-400" />,
  "?îß": <Settings className="h-8 w-8 text-gray-400" />,
  "?ì¶": <Package className="h-8 w-8 text-green-400" />,
...
const CATEGORIES: Category[] = [
  { name: "?¨Âè∏‰ªãÁ¥π", nameEn: "Company Introduction", count: 1 },
  { name: "?¨Âè∏?™Âã¢", nameEn: "Company Advantages", count: 1 },
  { name: "Â∑•ÂÖ∑?áÂ?", nameEn: "Tool Guide", count: 1 },
  { name: "ÁßëÊ??çÁûª", nameEn: "Tech Innovation", count: 1 },
  { name: "?∫Á??•Ë?", nameEn: "Fundamentals", count: 1 }
];

const POPULAR_TAGS = [
  "AI Formula", "?ÜÊ•≠?™Â???, "È¶ôÊ∏Ø‰∏≠Â?‰º?, "?∏‰?ËΩâÂ?", 
  "?çÂ??™Âã¢", "Â∞àÊ•≠?òÈ?", "Make.com", "?™Â??ñÂ∑•??
];
          ...
          <div className="space-y-4">
            ...
            <div className={`${isFeatured ? 'pt-4' : 'pt-3'}`}>
              <Link to={`/blog/${post.id}`}>
                ...
                >
                  {isZhTW ? (isFeatured ? '?±Ë??áÁ?' : '?±Ë??¥Â?') : (isFeatured ? 'Read Article' : 'Read More')}
...
const SearchBar: React.FC<{ searchState: SearchState; onSearchChange: (query: string) => void; isZhTW: boolean }> = ({ 
  ...
  <div className="max-w-md mx-auto relative">
    ...
    <Input
      placeholder={isZhTW ? "?úÂ??áÁ?..." : "Search articles..."}
    ...
    <div className="max-w-4xl mx-auto text-center">
      ...
      >
        <h3 className="text-3xl font-bold mb-4 text-white">
          {isZhTW ? 'Ë®ÇÈñ±AI Formula?ªÂ??? : 'Subscribe to AI Formula Newsletter'}
        </h3>
        <p className="text-gray-200 mb-8">
          {isZhTW 
            ? '?≤Â?AI Formula?Ä?∞Á?Â∞àÊ•≠Ë¶ãËß£?ÅÂØ¶?®Ê?Â≠∏Â?È¶ôÊ∏Ø?ÜÊ•≠Ê°à‰??ÜÊ?ÔºåÂä©?õÊÇ®?ÑAIËΩâÂ?‰πãË∑Ø??
        ...
        {!newsletterState.isSubscribed ? (
          <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
            <Input
              placeholder={isZhTW ? "Ëº∏ÂÖ•?®Á??ªÂ??µ‰ª∂?∞Â?" : "Enter your email address"}
            ...
            >
              {newsletterState.isLoading ? (isZhTW ? '?ïÁ?‰∏?..' : 'Processing...') : (isZhTW ? 'Ë®ÇÈñ±' : 'Subscribe')}
            </Button>
          </div>
        ) : (
          <div className="text-green-400 font-semibold">
            {isZhTW ? '??Ë®ÇÈñ±?êÂ?ÔºÅÊ?Ë¨ùÊÇ®?ÑÈ?Ê≥®„Ä? : '??Successfully subscribed! Thank you for your interest.'}
          </div>
        )}
      </motion.div>
    </div>
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <Badge variant="secondary" className="mb-4 bg-gradient-to-r from-blue-500/20 to-purple-500/20 text-blue-200 border-blue-400">
              {isZhTW ? '?Ä?∞Ë?Ëß???¥Êñ∞' : 'Latest Insights & Updates'}
            </Badge>
            <h1 className="text-4xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-blue-400 via-purple-500 to-cyan-400 bg-clip-text text-transparent">
              {isZhTW ? 'AI Formula Â∞àÂÆ∂Ë¶ãËß£' : 'AI Formula Expert Insights'}
            </h1>
            <p className="text-xl text-gray-200 max-w-3xl mx-auto mb-8">
              {isZhTW 
                ? '?¢Á¥¢AI Formula?ÑÂ?Ê•≠Ë?Ëß???åÊè°?Ä?∞AI?™Â??ñË∂®?¢Ô??≤Â?ÂØ¶Áî®?ÑÂ?Ê•≠Ê??®Ê??ó„Ä? 
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          >
            {isZhTW ? 'Á≤æÈÅ∏?áÁ?' : 'Featured Articles'}
          </motion.h2>
              ...
              <Card className="bg-gray-900/50 border-gray-700">
                <CardHeader>
                  <CardTitle className="flex items-center gap-2 text-white">
                    ...
                    {isZhTW ? '?áÁ??ÜÈ?' : 'Categories'}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <nav className="space-y-3" role="navigation" aria-label="?áÁ??ÜÈ?">
