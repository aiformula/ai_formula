...
const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  const popularCourses = [
    {
      ...
      descriptionCht: '4Â∞èÊ? ?∂Âü∫Á§éÂ≠∏?É„ÄéÁ?Ê≠?Ø¶?®„ÄèÁ?Á®ãÂ?Ë®≠Ë?Ôº?,
      price: 'HK$3,800',
    ...
    },
    {
      ...
      titleCht: 'Perplexity Â∑•ÂÖ∑??, 
      description: '3 Hours Become AI Search Research Super Expert!',
      descriptionCht: '3Â∞èÊ? ?êÁÇ∫AI?úÂ??îÁ©∂?éË?Á¥öÂ?ÂÆ∂„ÄèÔ?',
      price: 'HK$1,980',
  ...
  const allFilterTags = [
    // Category filters
    { key: 'all', label: 'All', labelCht: '?®ÈÉ®', emoji: '??', type: 'category' },
    { key: 'design', label: 'Creative Design', labelCht: '?µÊ?Ë®≠Ë?', emoji: '?é®', type: 'category' },
    { key: 'ai', label: 'AI Applications', labelCht: 'AI?âÁî®', emoji: '??', type: 'category' },
    { key: 'automation', label: 'Automation', labelCht: '?™Â???, emoji: '??, type: 'category' },
    { key: 'analytics', label: 'Data Analytics', labelCht: '?∏Ê??ÜÊ?', emoji: '??', type: 'category' },
    { key: 'prompt-engineering', label: 'Prompt Engineering', labelCht: '?êÁ§∫Â∑•Á?', emoji: '?í¨', type: 'category' },
    { key: 'programming', label: 'Programming', labelCht: 'Á®ãÂ?Ë®≠Ë?', emoji: '?íª', type: 'category' },
    // Industry filters
    { key: 'daily-life', label: 'Daily Life', labelCht: '?•Â∏∏?üÊ¥ª', emoji: '??', type: 'industry' },
    { key: 'insurance', label: 'Insurance', labelCht: '‰øùÈö™Ê•?, emoji: '?õ°Ô∏?, type: 'industry' },
    { key: 'retail', label: 'Retail', labelCht: '?∂ÂîÆÊ•?, emoji: '??Ô∏?, type: 'industry' },
    { key: 'finance', label: 'Finance', labelCht: '?ëË?Ê•?, emoji: '?í∞', type: 'industry' },
    { key: 'education', label: 'Education', labelCht: '?ôËÇ≤Ê•?, emoji: '??', type: 'industry' },
    { key: 'healthcare', label: 'Healthcare', labelCht: '?´Á?Ê•?, emoji: '?è•', type: 'industry' }
  ];
  ...
  const handleLeadSubmit = (e: React.FormEvent) => {
    ...
    alert(isZhTW ? 'Â§öË?‰Ω†Â??àË∂£ÔºÅÊ??ãÊ??°Âø´?ØÁµ°‰Ω†„Ä? : 'Thanks for your interest! We\'ll contact you soon.');
  ...
  const getLeadCollectionContent = () => {
    ...
    if (currentTag?.key === 'all') {
      return {
        title: isZhTW ? '?µÂ??∞ÊÉ≥Ë¶ÅÂ?Ë™≤Á?Ôº? : 'Can\'t find what you\'re looking for?',
        description: isZhTW 
          ? 'Ë©±‰øæ?ëÂ??•‰??≥Â≠∏?©Ô??ëÂ??ÉÁÇ∫‰Ω†Â∫¶Ë∫´Ë??†Ë™≤Á®ãÔ?'
          : 'Tell us what you want to learn, and we\'ll create a course just for you!',
        emailPlaceholder: isZhTW ? '‰Ω†Â??ªÈÉµ?∞Â?' : 'Your email address',
        textareaPlaceholder: isZhTW ? '‰Ω†ÊÉ≥Â≠∏Âí©Ë™≤Á?Ôºü‰?Â¶ÇÔ?Excel?≤È??ÄÂ∑ß„ÄÅPython?∏Ê??ÜÊ??ÅÊï∏‰ΩçÁ??∑Á??•Á?...' : 'What course would you like? e.g., Advanced Excel, Python Data Analysis, Digital Marketing...',
        buttonText: isZhTW ? '?ê‰∫§Ë™≤Á??ÄÊ±? : 'Submit Course Request',
        footerText: isZhTW ? '?ëÂ??ÉÂú®24Â∞èÊ??ßÂ?Ë¶Ü‰??ÖÈ?Ê±? : 'We\'ll respond to your request within 24 hours'
      };
    }
    ...
    return {
      title: isZhTW ? `??{categoryName}?çÂ?Ë™≤Á??ëÂ?Ê∫ñÂ?Á∑äÂ?ÔºÅ` : `"${categoryName}" course is coming soon!`,
      description: isZhTW 
        ? '?ßÂÆπÂ•ΩÂø´Â∞±Âà∞?ÇÊÉ≥?êÁÇ∫Á¨¨‰??πÂ≠∏?°Ô??ô‰?‰Ω†Â??ªÈÉµÔºåÊñ∞Ë™≤Á?‰∏Ä‰∏äÊû∂?≥Âàª?öÁü•‰Ω†Ô?'
        : 'Content coming soon. Want to be in the first batch? Leave your email and we\'ll notify you when the new course launches!',
      emailPlaceholder: isZhTW ? '‰Ω†Â??ªÈÉµ?∞Â?' : 'Your email address',
      textareaPlaceholder: isZhTW ? `‰Ω†Â?${categoryName}?âÂí©?πÂà•?≥Â≠∏?ÖÔ?ÔºàÈÅ∏Â°´Ô?` : `What specifically do you want to learn about ${categoryName}? (Optional)`,
      buttonText: isZhTW ? '?öÁü•?ëÊñ∞Ë™≤Á?‰∏äÊû∂' : 'Notify me when available',
      footerText: isZhTW ? '?∞Ë™≤Á®ã‰?‰∏äÊû∂?≥Âàª?öÁü•‰Ω? : 'We\'ll notify you as soon as the course is available'
    };
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold mb-4 text-white">
            {isZhTW ? 'Á≤æÈÅ∏?∏‰??¢Â?È°ûÂà•' : 'Featured Digital Product Categories'}
          </h2>
          <p className="text-gray-300 max-w-2xl mx-auto">
            {isZhTW 
              ? 'È´òË≥™Á¥†Á?Ë®òÂ?ÂΩ±Á??ôÂ≠∏ÔºåÂç≥?ª‰?ËºâÂ∞±?®Â???
            ...
            <div>
              <div className="text-center mb-8">
                <h3 className="text-2xl font-bold text-white mb-2 flex items-center justify-center gap-2">
                  ...
                  {isZhTW ? '?±È??®Ëñ¶Ë™≤Á?' : 'Popular Recommended Courses'}
                </h3>
                <p className="text-gray-300">
                  {isZhTW ? '?ºÊ??∂‰?Â≠∏Âì°?ΩÂ??õÂ?Ë™≤Á?' : 'Discover courses loved by other students'}
                </p>
              </div>
                      ...
                      <CardContent>
                        <div className="flex items-center gap-4 mb-3 text-sm text-gray-400">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <span>{course.students} {isZhTW ? 'Â≠∏Á?' : 'students'}</span>
                          </div>
                        </div>
