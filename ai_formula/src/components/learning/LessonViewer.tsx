...
const LessonViewer: React.FC<LessonViewerProps> = ({ lessonId }) => {
            ...
            <Card className="bg-white/5 border-gray-600">
              <CardHeader>
                <CardTitle className="text-white">
                  {isZhTW ? '課程描述' : 'Lesson Description'}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-[#E0E0E0] mb-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center p-2 bg-gray-800/50 rounded hover:bg-gray-800/70 cursor-pointer">
                      ...
                      <span className="text-[#E0E0E0] text-sm">
                        {isZhTW ? '課程介紹' : 'Course Introduction'}
                      </span>
                    </div>
                    <div className="flex items-center p-2 bg-gray-800/50 rounded hover:bg-gray-800/70 cursor-pointer">
                      <Clock className="w-4 h-4 text-[#3EFFDC] mr-2" />
                    ...
                    <div className="flex items-center p-2 bg-gray-800/50 rounded hover:bg-gray-800/70 cursor-pointer">
                      ...
                      <span className="text-[#E0E0E0] text-sm">
                        {isZhTW ? '實際應用' : 'Practical Applications'}
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <Card className="bg-white/5 border-gray-600">
              <CardHeader>
                <CardTitle className="text-white flex items-center">
                  ...
                  {isZhTW ? '互動文本內容' : 'Interactive Text Content'}
                </CardTitle>
              </CardHeader>
              <CardContent>
                <p className="text-[#E0E0E0] mb-4">
                ...
                {currentLesson.interactiveElements && currentLesson.interactiveElements.length > 0 && (
                  <div className="space-y-4">
                    <h3 className="text-white font-semibold">
                      {isZhTW ? '互動練習' : 'Interactive Exercises'}
                    </h3>
      ...
      default:
        return (
          <div className="text-center py-8">
            ...
            <p className="text-[#E0E0E0]">
              {isZhTW ? '課程內容正在加載中...' : 'Lesson content is loading...'}
            </p>
          </div>
        );
    }
  ...
  const InteractiveLearningInterface = () => {
    ...
    const handleAISubmit = async () => {
      ...
      // 模擬 AI 回應
      setTimeout(() => {
        setAiResponse(`AI 回應：${aiPrompt}\n\n這是一個示例回應。實際應用中會連接到真正的 AI 服務。`);
        setIsLoading(false);
      }, 1500);
    };
      ...
      <Card className="bg-white/5 border-gray-600">
        <CardHeader>
          <CardTitle className="text-white text-lg flex items-center">
            ...
            {isZhTW ? '互動學習界面' : 'Interactive Learning Interface'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as any)}>
            ...
            <TabsContent value="playground" className="space-y-4">
              <div className="space-y-3">
                <label className="text-sm font-medium text-[#E0E0E0]">
                  {isZhTW ? '輸入你的提示：' : 'Enter your prompt:'}
                </label>
                <textarea
                  value={aiPrompt}
                  onChange={(e) => setAiPrompt(e.target.value)}
                  placeholder={isZhTW ? '試試問 AI 一些問題...' : 'Try asking AI something...'}
              ...
              {aiResponse && (
                <div className="mt-4 p-4 bg-gray-800/70 rounded-lg text-sm text-[#E0E0E0] max-h-48 overflow-y-auto">
                  <div className="flex items-center mb-3">
                    ...
                    <span className="text-[#3EFFDC] font-semibold">AI 回應:</span>
                  </div>
            ...
            <TabsContent value="notes" className="space-y-4">
              <div className="space-y-3">
                <label className="text-sm font-medium text-[#E0E0E0]">
                  {isZhTW ? '添加筆記：' : 'Add note:'}
                </label>
                <div className="flex space-x-3">
                  <input
                    value={newNote}
                    onChange={(e) => setNewNote(e.target.value)}
                    placeholder={isZhTW ? '記錄重要內容...' : 'Record important content...'}
                  ...
                  <div className="p-4 bg-gray-800/50 rounded-lg">
                    <div className="flex items-center mb-3">
                      ...
                      <span className="text-sm font-semibold text-[#3EFFDC]">
                        {isZhTW ? '練習建議' : 'Exercise Suggestion'}
                      </span>
                    </div>
                    <p className="text-sm text-[#E0E0E0] leading-relaxed">
                      {isZhTW 
                        ? '試著根據課程內容寫一個提示，然後使用上面的 AI 練習功能測試效果。'
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[#E0E0E0]">
                        {isZhTW ? '完成狀態' : 'Completion Status'}
                      </span>
                      <span className={`text-sm ${isCompleted ? 'text-[#3EFFDC]' : 'text-gray-400'}`}>
                        {isCompleted ? (isZhTW ? '已完成' : 'Completed') : (isZhTW ? '進行中' : 'In Progress')}
                      </span>
                    </div>
                    ...
                    >
                      {isCompleted ? (
                        ...
                        <>
                          <Award className="w-4 h-4 mr-2" />
                          {isZhTW ? '標記完成' : 'Mark as Complete'}
                        </>
                      )}
                    </Button>
                    
                    {/* 下一課按鈕 - 只有在完成後且有下一課時顯示 */}
                    {isCompleted && nextLesson && (
                      ...
                      >
                        ...
                        {isZhTW ? `下一課：${nextLesson.titleZh}` : `Next: ${nextLesson.title}`}
                      </Button>
                    )}
                  </div>
                </CardContent>
